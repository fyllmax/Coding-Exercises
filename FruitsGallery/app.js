...
var groceries = [{
    ...
    // var template = "<div style='display:inline-block;'><% _().forEach(function(item) { %><h3><%= item.name %></h3> <img src='<%= item.image %>'><% }); %></div>";
...
function createGallery(items, type) {
    ...
    items.forEach(function(item) {
        ...
        template.push('<img src="', item.image, '" height="150" width="150" />');
        template.push('</div>');
    });
